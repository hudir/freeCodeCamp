...
echo "$($PSQL "SELECT course FROM students LEFT JOIN majors USING(major_id) LEFT JOIN majors_courses USING(major_id) LEFT JOIN courses USING(course_id) GROUP BY course HAVING COUNT(course)=1 ORDER BY course")"