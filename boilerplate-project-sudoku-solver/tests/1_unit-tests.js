...
suite("Unit Tests", () => {
  ...
  test("invalid characters", (done) => {
    assert.equal(
      new Solver().validate("a".repeat(81)).error,
  ...
  test("not 81 characters", (done) => {
    assert.equal(
      new Solver().validate("1".repeat(22)).error,
      "Expected puzzle to be 81 characters long"
    );
    assert.equal(
      new Solver().validate("9".repeat(66)).error,
      "Expected puzzle to be 81 characters long"
    );
    assert.equal(
      new Solver().validate(".".repeat(80)).error,
      "Expected puzzle to be 81 characters long"
    );
    done();
  });
  ...
  test("basic check", (done) => {
    assert.isOk(
      new Solver().checkBasic(
        ["1", "3", "5", "7", "6", "2", "9", "8", "4"].join("")
      )
    );
    assert.isNotOk(
      new Solver().checkBasic(
        ["1", "3", "5", "7", "6", "2", "9", "9", "4"].join("")
      )
    );
    done();
  });
  ...
  test("invalid column placement", (done) => {
    assert.isNotOk(
      new Solver().checkColPlacement(
        "987549163531672894649831527496157382218396475753284916962415738185763249374928651"
      )
    );
    done();
  });
  ...
  test("invalid region (3x3 grid) placement", (done) => {
    assert.isNotOk(
      new Solver().checkRegionPlacement(
        "987549163531672894649831527496157382218396475753284916962415738185763249374928651"
      )
    );
    done();
  ...
  const sudokuExpValid =
    "..9..5.1.85.4....2432......1...69.83.9.....6.62.71...9......1945....4.37.4.3..6..";

  const sudokuExpInvalid =
    "9.9..5.1.85.4....2432......1...69.83.9.....6.62.71...9......1945....4.37.4.3..6..";

  const solver = new Solver();

  test("Valid puzzle strings pass the solver", (done) => {
    assert.isOk(solver.solve(sudokuExpValid));
    assert.isString(solver.solve(sudokuExpValid));
    assert.lengthOf(solver.solve(sudokuExpValid), 81 );
  done();
});

  test("Invalid puzzle strings fail the solver", (done) => {
    assert.isFalse(solver.solve(sudokuExpInvalid));
  done();
});

  test("sudoku solving", (done) => {
